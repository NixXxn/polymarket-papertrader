...
def load_settings(
    ...
    return Settings(
        ...
        arbitrage=ArbitrageSettings(
            max_pair_cost=float(arbitrage_raw.get("max_pair_cost", 0.98)),
            max_maker_ask_sum=float(arbitrage_raw.get("max_maker_ask_sum", 1.06)),
            min_edge=float(arbitrage_raw.get("min_edge", 0.012)),
            fee_buffer=float(arbitrage_raw.get("fee_buffer", 0.005)),
            min_liquidity=float(arbitrage_raw.get("min_liquidity", 150.0)),
            min_volume_24h=float(arbitrage_raw.get("min_volume_24h", 50.0)),
            min_ask=float(arbitrage_raw.get("min_ask", 0.02)),
            max_ask=float(arbitrage_raw.get("max_ask", 0.98)),
            min_ask_size=float(arbitrage_raw.get("min_ask_size", 3.0)),
            book_fill_fraction=float(arbitrage_raw.get("book_fill_fraction", 1.0)),
            position_usd=float(arbitrage_raw.get("position_usd", 40.0)),
            max_position_usd=float(arbitrage_raw.get("max_position_usd", 80.0)),
            ...
            prefer_crypto_weather=bool(arbitrage_raw.get("prefer_crypto_weather", False)),
            prefer_lp_rewards=bool(arbitrage_raw.get("prefer_lp_rewards", True)),
            scan_limit=int(arbitrage_raw.get("scan_limit", 400)),
            starting_balance=(
        ...
        endgame=EndgameSettings(
            ...
            price_min=float(endgame_raw.get("price_min", 0.86)),
            price_max=float(endgame_raw.get("price_max", 0.97)),
            min_liquidity=float(endgame_raw.get("min_liquidity", 80.0)),
            min_ask_size=float(endgame_raw.get("min_ask_size", 4.0)),
            book_fill_fraction=float(endgame_raw.get("book_fill_fraction", 1.0)),
            use_full_capital=bool(endgame_raw.get("use_full_capital", False)),
            position_usd=float(endgame_raw.get("position_usd", 50.0)),
            max_position_usd=float(endgame_raw.get("max_position_usd", 100.0)),
            max_open_positions=int(endgame_raw.get("max_open_positions", 4)),
            ...
            poll_interval_seconds=int(endgame_raw.get("poll_interval_seconds", 12)),
            starting_balance=(
