...
@dataclass
class PennyExitState:
    market_slug: str
    take_profit_limit_placed: bool = False
    take_profit_price: float | None = None


class PennyExitStore:
    """Tracks resting 3¢ take-profit limits for penny positions."""

    def __init__(self, data_dir: Path | str) -> None:
        self._root = Path(data_dir)
        if self._root.name == "penny":
            self._root = self._root.parent
        self._path = self._root / "penny_exit_state.json"
        self._states: dict[str, PennyExitState] = {}
        self._load()
    ...
    def _load(self) -> None:
        ...
        for key, row in raw.items():
            ...
            self._states[key] = PennyExitState(
                market_slug=slug,
