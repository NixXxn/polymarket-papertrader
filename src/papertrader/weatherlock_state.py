...
@dataclass
class WeatherlockExitState:
    market_slug: str
    take_profit_limit_placed: bool = False
    take_profit_price: float | None = None


class WeatherlockExitStore:
    """Tracks resting 99¢ take-profit limits for weatherlock NO positions."""

    def __init__(self, data_dir: Path | str) -> None:
        self._root = Path(data_dir)
        if self._root.name == "weatherlock":
            self._root = self._root.parent
        self._path = self._root / "weatherlock_exit_state.json"
        self._states: dict[str, WeatherlockExitState] = {}
        self._load()
    ...
    def _load(self) -> None:
        ...
        for key, row in raw.items():
            ...
            self._states[key] = WeatherlockExitState(
                market_slug=slug,
