...
@dataclass
class EndgameExitState:
    market_slug: str
    take_profit_limit_placed: bool = False
    take_profit_price: float | None = None


class EndgameExitStore:
    """Tracks resting take-profit limits for endgame sports locks."""

    def __init__(self, data_dir: Path | str) -> None:
        self._root = Path(data_dir)
        if self._root.name == "endgame":
            self._root = self._root.parent
        self._path = self._root / "endgame_exit_state.json"
        self._states: dict[str, EndgameExitState] = {}
        self._load()
    ...
    def _load(self) -> None:
        ...
        for key, row in raw.items():
            ...
            self._states[key] = EndgameExitState(
                market_slug=slug,
